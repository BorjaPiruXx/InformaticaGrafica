...
void render(const Shader& shader1, const Shader& shader2, const Geometry& figure1, const Geometry& figure2, const Geometry& figure3, const Texture& texture1, const Texture& texture2, const Texture& texture3)
{
    ...
    glm::vec3 lightPosition = glm::vec3(1.0f, 2.0f, 1.0f);
    ...
    for(size_t i = 0; i < spherePositions.size(); ++i)
    {
        ...
        if(i < 2)
        {
            if(i == 0)
            {   
                texture1.use(shader2, "material.diffuse", i);
            } else{
                texture2.use(shader2, "material.specular", i);
            }
        }
...
int main(int, char*[]) 
{
    ...
    const Sphere gouraudSphere(0.5f, 50, 50);
    const Sphere flatSphere(0.5f, 50, 50);
    const Sphere phongSphere(0.5f, 50, 50);
    ...
    while(window->isAlive())
    {
        ...
        //update();
        render(lightShader, phongShader, gouraudSphere, flatSphere, phongSphere, texture1, texture2, texture3);
        window->frame();
    }
