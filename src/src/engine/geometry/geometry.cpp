...
void Geometry::calcTangents(const float* positions, const float* uvs, const float* normals, float* tangents, float* biTangents) const 
{
    ...
    // Cálculo de tangentes para vértices
...
void Geometry::uploadData(const float* positions, const float* uvs, const float* normals, const uint32_t* index) 
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t) * _nElements, index, GL_STATIC_DRAW);
