...
Texture::Texture(const char* path, Format format) : _format(format) 
{
    ...
    unsigned char* image = stbi_load(path, &_width, &_height, &_depth, 0);
    if(image) 
    {
        glTexImage2D(GL_TEXTURE_2D, 0, format2GL(_format), _width, _height, 0, format2GL(_format), GL_UNSIGNED_BYTE, image);
