...
class Teapot final :public Geometry {
    ...
    private:
        ...
        static void buildPatchReflect(uint32_t patchNum, float* B, float* dB, float* positions, float* uvs, float* normals, uint32_t* indices, uint32_t& idx, uint32_t& uvIdx, uint32_t& indicesIdx, int grid, bool reflectX, bool reflectY);

        static void buildPatch(glm::vec3 patch[][4], float* B, float* dB, float* positions, float* uvs, float* normals, uint32_t* indices, uint32_t& idx, uint32_t& uvIdx, uint32_t& indicesIdx, uint32_t grid, glm::mat3 reflect, bool invertNormal);
